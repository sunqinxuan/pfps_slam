...
//			vis->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 4, id);
	...
	void display(Scan *scan_cur, int index,  boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
		char id[20];
		sprintf(id,"spt%d",index);
		Transform Tgc;
		Eigen::Matrix4f trans;
		pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_show_tmp(new pcl::PointCloud<pcl::PointXYZRGBA>);
//		vis->removeAllPointClouds();
		pcl::VoxelGrid<pcl::PointXYZRGBA> vg;
		vg.setInputCloud(scan_cur->point_cloud);
		vg.setLeafSize(0.03,0.03,0.03);
		vg.filter(*cloud_show_tmp);
...
//		}
		Tgc=scan_cur->Tcg_gt->inv();
		trans=Tgc.getMatrix4f();
		transformPointCloud(*cloud_show_tmp,*cloud_show_tmp,trans);
		if (!vis->updatePointCloud (cloud_show_tmp, id))
			vis->addPointCloud (cloud_show_tmp, id);
	}
