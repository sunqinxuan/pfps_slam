...
#
# Last modified: 2017-06-05 14:43
#
...
namespace ulysses
{
	...
	bool InterpTree::Construct(std::vector<Plane*> &planes_ref_,std::vector<Plane*> &planes_cur_)
	{
		...
		if(debug)
		{
			...
			{
				std::cout<<planes_cur_[i]->index<<","<<planes_cur_[i]->coefficients.transpose()<<std::endl;
			}
			std::cout<<"plane_ref:"<<planes_ref_.size()<<std::endl;
			for(int i=0;i<planes_ref_.size();i++)
			{
				std::cout<<planes_ref_[i]->index<<","<<planes_ref_[i]->coefficients.transpose()<<std::endl;
			}
		}
				...
				for(int i_node=nodes.size()-1;i_node>=0;i_node--)
				{
					...
					//if((*it_node)->plane_cur->index==tmp_node->plane_cur->index-1)
					{
						...
						if(flag_consistent)
						{
							if(debug)
							{
								std::cout<<"insert <"<<tmp_node->plane_cur->index<<","<<tmp_node->plane_ref->index<<"> to <"<<node_attach->plane_cur->index<<","<<node_attach->plane_ref->index<<">"<<std::endl;
							}
							insert_node=new Node_InterpTree(*it_ref,*it_cur);
...
//		Eigen::Vector3d tmp;
//		tmp=node->plane_cur->centroid_rgb-node->plane_ref->centroid_rgb;
//		if(debug)
//		{
...
//			return false;
	...
	bool InterpTree::consistent_2(Node_InterpTree *node1, Node_InterpTree *node2)
	{
		...
		cos_angle_cur=node1->plane_cur->coefficients.block<3,1>(0,0).transpose()*node2->plane_cur->coefficients.block<3,1>(0,0);
		cos_angle_ref=node1->plane_ref->coefficients.block<3,1>(0,0).transpose()*node2->plane_ref->coefficients.block<3,1>(0,0);
		angle_cur=acos(cos_angle_cur);
		angle_ref=acos(cos_angle_ref);
		if(angle_cur<thres_delta_angle)
		{
			dist_cur=abs(node1->plane_cur->coefficients(3)-node2->plane_cur->coefficients(3));
		}
		else
		{
			dist_cur=0;
		...
		{
			dist_ref=abs(node1->plane_ref->coefficients(3)-node2->plane_ref->coefficients(3));
		}
		else
		{
			dist_ref=0;
	...
	void PlaneFeatureMatching::match(std::vector<Plane*> planes_ref,std::vector<Plane*> planes_cur, 
